        ...
        <div id="profile" class="dropdown">
            @if(Auth::check())
                ...
                <div class="dropdown-menu" aria-labelledby="profile-button">
                    <a class="dropdown-item {{ Route::currentRouteName() == 'profile' ? 'active' : '' }}" href="{{ route('profile', ['username' => $user->username]) }}"><h2>See Profile</h2></a>
                    <a class="dropdown-item {{ Route::currentRouteName() == 'notifications.show.page' ? 'active' : '' }}" href="{{ route('notifications.show.page') }}"><h2>Notifications</h2></a>
                    <a class="dropdown-item {{ Route::currentRouteName() == 'showFavouriteArticles' ? 'active' : '' }}" href="{{ route('showFavouriteArticles') }}"><h2>Favourite Articles</h2></a>
                    @if($user->is_admin)
                        <a class="dropdown-item {{ Route::currentRouteName() == 'adminPanel' ? 'active' : '' }}" href="{{ route('adminPanel') }}"><h2>Administrator Panel</h2></a>
                    @endif
    ...
    <div id="bottom-part-header">
        ...
        <div class="dropdown-menu" aria-labelledby="all-topics-button">
            @foreach($topics as $topic)
                <a class="dropdown-item {{ Route::currentRouteName() == 'showTopic' && Route::input('name') == $topic->name ? 'active' : '' }}" href="{{ route('showTopic', ['name' => $topic->name]) }}"><h2>{{ $topic->name }}</h2></a>
            @endforeach
        </div>
